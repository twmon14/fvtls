...
\end{verbatim}
\end{small}
In that case, (1) is semantically rewritten into (3) and (2) is rewritten into four sub-equations. Moreover, the equation (4) is required to use as an assumption. Then, CafeOBJ returns true for this case. The case in which k is different from k(enemy), however, cannot be reduced into true, In that case, an additional equation \verb!b = enemy! is required for case-splitting, the equation (3) is rewritten into \verb!enc(k,r1,enermy) \in ciphers(nw(s)) = true! and the second invariant, inv2 is used as the induction hypothesis to deduce that.
% \begin{small}
% \begin{verbatim}
% open IFF .
% op a : -> Prin . op b : -> Prin . op c : -> Prin .
...
The projection operators are defined to return the arguments of the data constructors. The projection operators such as \verb!crt!, \verb!src! and \verb!dst! return the first, second and third arguments of a message, respectively. 
...
The behavior of trustable principals is defined by 12 kinds of transitions corresponding to sending 10 kinds of messages and receiving two Finished messages respectively;
...
\section{Revising semantic-based case splitting and one Missing lemma}
In the existing proof scores of TLS protocol, there are many times case splitting based on semantics are used. 
...
For example, the original proof score for the induction case  \verb!shello! of inv1 consists of the following two open-close fragments:
...
CiMPA is easy to choose an erroneous order for the case distinctions, generating more goals than required. 
