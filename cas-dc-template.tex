...
% We will present in the next sections how proof scores can be formally checked that no human error is lurking in the proof scores by using CiMPA and CiMPG.
...
% op d : -> Prin . op e : -> Prin . op r2 : -> Cipher .
% eq r2 \in ciphers(nw(s)) = true .
% eq a = enemy .  eq d = b .  eq e = c .
% eq k(r2) = k .  eq r1(r2) = r1 .    eq p(r2) = b .
% eq k = k(enemy) .
% red inv1(s,a,b,c,k,r1) implies inv1(fkrm1(s,d,e,r2),a,b,c,k,r1) .
% close
% \end{verbatim}
% \end{small}
% Here, we have proven a very specific case in which several equations hold. We need to prove all possible cases to really prove the invariant. For example, we can assume that k is different from k(enemy). In that case, two additional equations are required and the second invariant is used as the induction hypothesis to deduce that.
% \begin{small}
% \begin{verbatim}
% open IFF .
% op a : -> Prin . op b : -> Prin . op c : -> Prin .
...
% eq k(r2) = k .  eq r1(r2) = r1 .    eq p(r2) = b .
% eq (k = k(enemy)) = false .
% eq b = enemy . eq enc(k,r1,enemy) \in ciphers(nw(s)) = true .
% red inv2(s,k,r1) implies inv1(s,a,b,c,k,r1) 
% implies inv1(fkrm1(s,d,e,r2),a,b,c,k,r1) .
% close
% \end{verbatim}
% \end{small}
% Similarly, we would write proof scores for covering all cases as shown in the table \ref{}; in this case, we used 11 open–close environments for checking them; all are available at the repository given in the previous
% section. Note that it is easy to develop proofs with proof scores, but it is also subject to human errors which leads to the case that we would run the complete set of proof scores obtaining true for all of them, but the property would not be really proven. We will
% present in the next sections how proof scores can be formally checked that no human error is lurking in the proof scores.
...
%\begin{itemize}
%	\item $csb1 \triangleq$ \verb!r \in ur(p)!
%	\item $csb2 \triangleq$ \verb!i \in ui(p)!
%	\item $csb3 \triangleq$ \verb!m \in nw(p)!
%	\item $csb4 \triangleq$ \verb!ch?(m)!
%	\item $csb5 \triangleq$ \verb!dst(m) = b!
%	\item $csb6 \triangleq$ \verb!c \in list(m)!
%\end{itemize}
