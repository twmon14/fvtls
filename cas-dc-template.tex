...
Internet security is extremely crucial nowadays because many kinds of credential information, such as credit card numbers and social security numbers, are transmitted over the Internet. To make such transmission secure, many security protocol have been designed and developed so far. One of the most widely used ones is Transport Layer Security (TLS) [\cite{dierk}] that is the successor of Secure Sockets Layer (SSL) [\cite{*1}]. 
% here
...
\end{verbatim}
\end{small}	
where constant \verb!void! denotes the empty bag. Operator    \verb|_,_| of \verb!M,NW! is the data constructor of bags. The first equation says that no other encrypted pre-master secrets are at any initial state. Messages from which encrypted pre-master secrets can be gleaned are KeyExchange messages only. The second equation says that if there exits a KeyExchange message in the network and the pre-master secret in the message is not encrypted with the intruder’s public key, then the encrypted pre-master secret in the message can be available to the intruder. The third one says that no encrypted pre-master secrets cannot be gleaned from any non-KeyExchange messages and any KeyExchange messages whose pre-master secrets are encrypted with the intruder’s public key. The remaining operators are also defined likewise.
...
In the original specification of the protocol, the state space is denoted by hidden sort and the transition and observation operators with respect to hidden sorts are declared by starting with \verb !bop!. 
...
\end{verbatim}
\end{small}
where \verb!tls! is just an identifier, can be replaced by another preferred one. Feeding the annotated proof scores into CiMPG, CiMPG automatically generates the proof script for CiMPA.
